...
<section class="flex flex-col grow w-full h-full bg-cassian-primary max-w-94.25 xl:max-w-cassian-1440 mx-auto">
    <p class="font-cassian-inter text-[10px] font-light text-cassian-gray xl:mt-4 hidden xl:flex xl:ml-37.5">
    ...
    <div class="flex flex-col xl:flex-row justify-center xl:justify-start items-start w-full xl:mt-4">
        ...
        <div
            class="flex flex-col justify-start items-start bg-cassian-secondary order-2 xl:order-2 
            pt-[40.52px] xl:pt-14.75 px-5 xl:px-21.25 pb-20 w-full xl:h-215.75">
            ...
            <h3 class="font-cassian-inter font-semibold text-[8px] text-cassian-black-light mt-[32.5px]">
                DESCRIPTION
            </h3>
            <p class="font-cassian-inter text-[14px] w-83.5 xl:w-121.25 xl:h-72.25 mt-4">
                <?= htmlspecialchars($book->getDescription() ?? '') ?>
            </p>
            <h3 class="font-cassian-inter font-semibold text-[8px] text-cassian-black-light mt-10 xl:mt-8">
                PROPRIÉTAIRE
            </h3>
            <a href="/public-account?pseudo=<?= htmlspecialchars($user->getPseudo() ?? '') ?>">
                <div
                    class="flex rounded-[114px] w-39.25 h-15 bg-cassian-white mt-4 pl-1.5 gap-3 
                    ...
                    <span
                        class="font-cassian-inter text-[14px]"><?= htmlspecialchars($user->getPseudo() ?? 'Inconnu') ?>
                    </span>
                </div>
            </a>
            <a href="/new-message?to=<?= htmlspecialchars($book->getUserId()) ?>"
                class="flex justify-center items-center w-83.5 xl:w-121.25 h-15.75 transition-colors 
                duration-300 ease-in-out bg-cassian-green hover:bg-cassian-green-strong font-cassian-inter 
                font-semibold text-cassian-white text-[16px] rounded-[10px] mt-10 xl:mt-20">
                Envoyer un message
            </a>
        </div>
    </div>
