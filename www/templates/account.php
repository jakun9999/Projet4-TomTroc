...
<section class="grow w-full h-full bg-cassian-secondary max-w-94.25 xl:max-w-cassian-1440 mx-auto">
    <div class="flex flex-col justify-center ml-5 xl:ml-37.5">
        <h1 class="font-cassian-playfair text-[30px] xl:text-[26px] text-cassian-black w-full mt-19.5 xl:mt-22.5">Mon
            compte</h1>
        ...
        <div class="flex flex-col xl:flex-row gap-8 xl:gap-8.25 mt-10 xl:mt-12">
            ...
            <div
                class="flex flex-col gap-12 pt-12 pb-9 xl:pb-23.25 rounded-[20px] items-center w-83.75 xl:w-136.75 h-112.75 xl:h-127 bg-cassian-white">
                ...
                <!-- Account summary -->
                <div class="flex flex-col items-center">
                    <h2 class="font-cassian-playfair text-cassian-black-light text-[24px]"><?= $user->getPseudo(); ?>
                    </h2>
                    <p class="font-cassian-inter text-cassian-gray text-[14px] mt-2.75">
                        Membre depuis <?= Utils::age($user->getCreationDate()) ?>
                    </p>
                    <h3
                        class="font-cassian-inter font-semibold text-cassian-black-light text-[8px] tracking-[0.64px] mt-5.25">
                        BIBLIOTHEQUE</h3>
                    <p class="flex justify-center items-center mt-1.5">
                        <span
                            class="shrink-0 w-[10.41px] h-[13.71px] bg-current inline-block mask-library text-cassian-black-light"></span>
                        <span class="font-cassian-inter text-[14px] text-cassian-black-light">&nbsp;
                            <?= htmlspecialchars($booksCount) ?>
                            <?= $booksCount > 1 ? 'livres' : 'livre' ?>
                        </span>
                    </p>
                    <a href="/new-book" class="font-cassian-inter text-[14px] text-cassian-black-light mt-3">Ajouter un
                        livre</a>
                </div>
            </div>
            ...
            <div
                class="flex flex-col rounded-[20px] items-center justify-center w-83.75 xl:w-136.75 h-129.75 xl:h-127 bg-cassian-white">
                <div class="w-67.25 xl:w-80.5 flex flex-col items-start">
                    <h3 class="text-left font-cassian-inter text-[16px] text-cassian-black-light">Vos informations
                        personnelles</h3>
                    <form action="/update-account" method="POST"
                        ...
                        <label for="email" class="font-cassian-inter text-cassian-gray text-[14px]">Adresse
                            email</label>
                        <input type="email" value="<?= htmlspecialchars($user->getEmail()) ?>" id="email" name="email"
                            required
                            class="pl-3.5 focus:outline-cassian-green bg-cassian-gray-strong h-12.5 font-cassian-inter text-[14px] border border-cassian-border-form rounded-md mt-2.5 w-full xl:w-80.5">
                        ...
                        <label for="password" class="font-cassian-inter text-cassian-gray text-[14px] mt-8">Mot de
                            passe</label>
                        ...
                        <input type="text" value="<?= htmlspecialchars($user->getPseudo()) ?>" id="pseudo" name="pseudo"
                            required
                            class="pl-3.5 focus:outline-cassian-green bg-cassian-gray-strong h-12.5 font-cassian-inter text-[14px] border border-cassian-border-form rounded-md mt-2.5 w-full xl:w-80.5">
                        <?php
                            if ($pseudoMessage !== '') {
                        ...
                        <?php
                            ...
                            ?>
                        <button type="submit"
                            class="mt-8 h-15.75 w-full xl:w-37.5 text-center font-cassian-inter inline-block bg-cassian-primary hover:text-cassian-white border-cassian-green border text-cassian-green font-semibold text-base rounded-[10px] py-4 transition-colors duration-300 ease-in-out hover:bg-cassian-green-strong">
                            Enregistrer
                        </button>
                    </form>
                </div>
