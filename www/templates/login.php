    ...
    <div class="flex flex-col xl:flex-row justify-center items-start w-full">
        <div class="flex flex-col justify-center items-start order-1 xl:order-1 px-5 xl:px-0 xl:ml-37.5 w-full xl:w-auto">
            ...
            <form action="/authenticate" method="POST" class="flex flex-col mt-8 xl:mt-14 xl:w-80.5 w-full">
                ...
                <input type="email" value="<?= htmlspecialchars($emailOldValue) ?>" id="email" name="email" required class="pl-3.5 bg-cassian-white h-12.5 font-cassian-inter text-[14px] border border-cassian-border-form focus:outline-cassian-green rounded-[10px] mt-2.5 w-full xl:w-80.5">
                <label for="password" class="font-cassian-inter text-cassian-gray text-[14px] mt-8">Mot de passe</label>
                <input type="password" id="password" name="password" required class="pl-3.5 bg-cassian-white font-cassian-inter text-[14px] h-12.5 border focus:outline-cassian-green border-cassian-border-form rounded-[10px] mt-2.5 w-full xl:w-80.5">
                <?php
                /**
