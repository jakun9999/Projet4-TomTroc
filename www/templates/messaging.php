...
$selectedDiscussionMessages = $params['selected_discussion_messages'] ?? [];

// Used to display first discussion as selected if no
// discussion is selected by the user.
$discussionCount = 0;
...
max-w-94.25 xl:max-w-cassian-1440 mx-auto">
    <div class="flex flex-1 xl:w-285 min-h-full">
        ...
        <div class="flex flex-col w-83.75 xl:w-77 min-h-full bg-cassian-secondary">
            ...
            <div class="flex flex-col mt-6.75">
                <?php foreach ($discussions as $discussion): ?>
                    ...
                    if (!is_null($discussion->getId()) && $discussion->getId() === $selectedDiscussion->getId()) {
                    ...
                    ?>
                    <a href="<?= is_null($discussion->getId()) ? '#' : '/show-discussion?with=' . $discussion->getOtherUserId() ?>"
        ...
        <!-- Discussion messages -->
        <div class="hidden xl:flex flex-col w-83.75 xl:w-208 xl:flex-1 min-h-full xl:pl-11 xl:pt-8.75 xl:pb-24.5 bg-cassian-secondary
         xl:bg-cassian-primary">
            ...
            <?php else: ?>
                ...
                <!-- Messages -->
                <div class="flex flex-row xl:w-191 xl:grow xl:overflow-y-auto xl:pb-24.5">
                    <?php foreach ($selectedDiscussionMessages as $message): // Message from current user
                    ?>
                ...
                <!-- new message form -->
                <form action="/send-message?to=<?= htmlspecialchars($selectedDiscussion->getOtherUserId()) ?>"
                    method="GET"
                    ...
                    <input type="text" id="message" name="message" placeholder="Tapez votre message ici"
                        class="w-83.75 xl:w-157 h-12.25 bg-cassian-white rounded-md border border-cassian-border-form
                            font-cassian-inter text-[14px] px-5.5 xl:px-10.5 ml-1.5 focus:outline-cassian-green">
                    <?php
                    /**
